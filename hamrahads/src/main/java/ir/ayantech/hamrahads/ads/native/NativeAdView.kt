...
//                                    )
            ...
            if (!tracked && isViewVisibleEnough(view)) {
                ...
                ioScope.launch {
                    native.trackers?.impression?.let {
                        when (NativeRepository(NetworkClient(activity.applicationContext)).impression(
                            it
                        )) {
    ...
    private fun onClickView(native: NativeAdDto, activity: AppCompatActivity) {
        if (!isClick) {
            isClick = true
            ioScope.launch {
                native.trackers?.click?.let {
                    when (val result =
                        NativeRepository(NetworkClient(activity.applicationContext)).click(it)) {
