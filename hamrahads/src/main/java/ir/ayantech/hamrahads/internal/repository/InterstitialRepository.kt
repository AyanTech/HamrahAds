...
class InterstitialRepository(private val networkClient: NetworkClient) {

    private val dataFetcher = NetworkDataFetcher()

    suspend fun fetchInterstitialAds(
        zoneId: String,
        deviceInfo: DeviceInfo
    ): NetworkResult<InterstitialAdDto> {
        return dataFetcher.fetchData {
            networkClient.createApiService().getInterstitialAds(
                zoneId = zoneId,
